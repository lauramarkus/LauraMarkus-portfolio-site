...
const inputCheck = function(input){
    ...
    } else if (input.length > 1){ //checks if there is more than 1 letter
        message.innerText = "Just 1 letter at a time."; 
    } else if (!input.match(acceptedLetter)){ //makes sure input is not a special character
        message.innerText = "You can only guess letters from A to Z. Let's try again."; 
    } else {
    return input;
}
};
